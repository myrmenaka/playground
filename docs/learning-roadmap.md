...
5. PHP独自の重要要素(`==/===`、nullable、`strict_types`、リクエストモデル、スーパーグローバル、Enum)を理解し、Laravel学習の準備ができている
...
- **PHP-B2-S18**: 例外の基本構文(`throw` / `try-catch-finally` / カスタム例外)(写経→再現)
- **PHP-B2-S19**: 在庫管理クラス(`Product` + 複数種類の例外を投げ分ける)(自力)
- **PHP-B2-S20**: 分解して書く手順を身につける(図書館の貸し出し管理)(自力)
...
1. `==` と `===` の違いを5パターン以上のコードで説明できる
2. nullable 型 と Union 型 の使い分けができる
3. `declare(strict_types=1)` が何を保証しているかを説明できる
4. PHP の「リクエストごとプロセスモデル」を説明できる
5. スーパーグローバル変数の役割を理解し、Laravel での隠蔽方法を知っている
6. PHP 8.1 Enum を使ったコードが書ける

### 等値比較の罠

- **PHP-B3-S1**: `==` と `===` の違いを5パターンで確認するコードを書く(写経→自力)
  - `"0" == false`、`"abc" == 0`、`null == false`、`[] == false`、数値文字列の比較など
  - 各パターンで何が起きるかをコメントで明示

### 型システム

- **PHP-B3-S2**: nullable 型(`?string`)と Union 型(`int|string`)を使った関数を書く(写経→自力)
  - nullable の null チェック
  - Union 型での型分岐

### モダンPHP の作法

- **PHP-B3-S3**: `declare(strict_types=1)` を使ったファイルと使わないファイルで型チェックの動作を比較(写経→概念整理)
  - 型強制と型エラーの違いを実際に動かして確認
  - 自分の言葉でノートアプリに整理

### PHP のリクエストモデル

- **PHP-B3-S4**: PHP の「リクエストごとプロセスモデル」を AI に質問してノートに整理(概念Step)
  - 常駐プロセス型の言語との違いを認識する
  - なぜセッションや DB が必要なのか、その必然性を理解する

### スーパーグローバル

- **PHP-B3-S5**: スーパーグローバル変数(`$_GET`、`$_POST`、`$_SESSION`)を使った簡単なフォーム処理を書く(写経→自力)
  - 素の PHP でフォーム処理を書く
  - Phase 1 で Laravel がこれをどう隠蔽するか比較できる土台にする

### Enum(PHP 8.1+)

- **PHP-B3-S6**: PHP 8.1 の Enum を使ったコードを書く(写経→自力)
  - 基本 Enum(`enum Status { case Active; case Inactive; }`)
  - Backed Enum(`enum Status: string { case Active = 'active'; }`)
  - メソッドを持つ Enum

### Block 3 完了テスト

- **PHP-B3-T1**: 「商品の在庫状態を表す Enum(`InStock`、`LowStock`、`OutOfStock`)を作り、`Product` クラスで使う。`Product` の `status` プロパティは nullable で初期値 null。`declare(strict_types=1)` を冒頭に書き、`===` で状態判定する関数を書く」

  Block 2 で身につけたクラス設計と、Block 3 で身につけた PHP 独自要素を組み合わせる総合課題。

### このBlockで「やらない」こと

ロードマップ v2.1 まで予定していた以下の Step は、優先度を見直して **割愛** または **別の場所に統合** した:

- 配列の Java/PHP 対応(Block 2 までで実質的に習得済み)
- 配列操作の対応表メモ(`references/eng_cheatsheet.md` で代替可能)
- `match` 式と Java switch の比較(必要なら Phase 1 で出会った時に学ぶ)
- Java と PHP の差分10個リストアップ(Java を業務で使う予定がないため)
...
- **v2.2 (本版)**: Block 2 実施を踏まえて以下を反映:
  ...
  - Phase 0 - Block 3 を「PHPとJavaの差分を体感する」(12Step)から「PHP独自の重要要素」(6Step)に縮小再設計
    ...
    - 業務頻度と Phase 1 への接続性を基準に Step を厳選
